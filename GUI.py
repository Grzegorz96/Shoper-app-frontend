...
def init_login_window(top_panel_frame, root):
    ...
    login_button = Button(login_window_label, text="Zaloguj się", font=("Arial", 10), borderwidth=0, bg="#D3D3D3",
                          command=lambda: My_functions.login_user(entry_login_or_email, entry_password, login_window,
                                                                  top_panel_frame, init_shopper_page_frame, root))
    ...
    combobox_day_birthday = ttk.Combobox(login_window_label, textvariable=combobox_day_var, width=6)
    ...
    combobox_year_birthday = ttk.Combobox(login_window_label, textvariable=combobox_year_var, width=6)
    ...
    # image_register = PhotoImage(file="register.png")
    # image_register_label = Label(login_window_label, image=image_register)
    # image_register_label.grid(row=17, column=0, pady=(0, 10), sticky=E)
...
def init_edit_user_announcement_page_frame(user_active_announcement_object, root):
    Config_data.current_page.destroy()
...
def init_messages_page_frame(root):
    if Config_data.is_user_logged_in:
        ...
        def update_text_and_name_label(conversation_object, is_user_customer):
            def set_right(event):
